function [Trials]= edfExtractInterestingEvents(Trials)
...
%% finding start of recording
  Trials(iTrial).StartTime= edfFindTrialRecordingStart(Events);
  ...
  for iE= 1:length(Trials(iTrial).Events.type)
    switch (Trials(iTrial).Events.type(iE))
      ...
      case 6 %% Saccade end
        ...
        Saccades.time(iS)= Saccades.entime(iS)-Saccades.sttime(iS);
      ...
      case 8 %% Fixation end
        ...
        Fixations.time(iF)= Fixations.entime(iF)-Fixations.sttime(iF);
