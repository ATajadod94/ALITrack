function [varargout,clickedLine] = clickableLegend(varargin)
...
% MODIFIED HERE TO INITIALIZE THE GUIDATA TO STORE INDEXES OF ALL LINES!!==
% Get indexes of all figures
allAxesInFigure = findall(gcf,'type','axes');
%axesHName=strcat('axesH_',num2str(allAxesInFigure(i)));

% Store line indexes to the axes!
if isempty(guidata(gca)),
    for i=1:length(allAxesInFigure)
        axesClicked.(strcat('AH_',num2str(i)))=ones(1,length(obj));
    end
    guidata(gca,[axesClicked]);
end;
...
for i = 1:length(plothan)
    set(objhan(i), 'HitTest', 'on', 'ButtonDownFcn',...
        @(varargin)togglevisibility(objhan(i),obj(groupmem==groups(i)),obj),...
        'UserData', true);
    if dispflag && hidden(i)
        clickedLine=togglevisibility(objhan(i), obj(groupmem==groups(i)),obj);
    else
        clickedLine=[];
    end
end
...
function clickedLine=togglevisibility(hObject, obj,allobj)
...
if isempty(guidata(gca)),
    for i=1:length(allAxesInFigure)
        axesClicked.(strcat('AH_',num2str(i)))=ones(1,length(allobj));
    end
    guidata(gca,[axesClicked]);
end;


%This is added to get indeces of clicked lines...
curGuiDataAll=guidata(gca);clickedLine=find(obj==allobj);
curGuiData=curGuiDataAll.(strcat('AH_',num2str(find(allAxesInFigure==gca))));
...
if get(hObject, 'UserData') % It is on, turn it off
    ...
    curGuiData(clickedLine)=0;
else
    set(hObject, 'Color', get(hObject, 'Color')*1.5 - 1, 'UserData', true);
    set(obj, 'HitTest','on','visible','on','handlevisibility','on');
    curGuiData(clickedLine)=1;
end

% Store them in axes...
curGuiDataAll.(strcat('AH_',num2str(find(findall(gcf,'type','axes')==gca))))=curGuiData;
guidata(gca,curGuiDataAll)
